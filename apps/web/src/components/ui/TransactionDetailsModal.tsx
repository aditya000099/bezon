        ...
        <div className="p-5 space-y-6">
          ...
          <div className="flex justify-between items-center bg-zinc-50 rounded-xl p-4 border border-zinc-100">
            <div>
              ...
              <p
                className={`text-2xl font-black mt-1 font-mono ${isCredit ? "text-emerald-600" : "text-rose-600"}`}
              >
                {isCredit ? "+" : "-"}₹
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              <p className="text-xs font-bold text-zinc-400 uppercase tracking-wider mb-1 flex items-center gap-1">
                <ClockIcon className="h-3 w-3" /> Timestamp
              </p>
            ...
            {order?.shopName && (
              <div>
                <p className="text-xs font-bold text-zinc-400 uppercase tracking-wider mb-1 flex items-center gap-1">
                  <TagIcon className="h-3 w-3" /> Seller
                </p>
              ...
              <div className="p-4 space-y-3 bg-white text-sm">
                ...
                {order.commissionDeducted > 0 && (
                  <div className="flex justify-between items-center text-rose-600">
                    ...
                    <span className="font-semibold">
                      -₹{order.commissionDeducted?.toLocaleString() || 0}
                    </span>
                  </div>
                )}
                ...
                <div className="pt-3 mt-1 border-t border-zinc-100 flex justify-between items-center">
                  ...
                  <span className="font-black text-emerald-600 text-base">
                    ₹{order.settlementAmount?.toLocaleString() || 0}
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="bg-zinc-50 p-4 rounded-xl flex items-center justify-between text-sm border border-zinc-100">
            <div>
              ...
              <p className="font-mono text-zinc-600">
                ₹{Number(transaction.balanceBefore).toLocaleString()}
              </p>
            </div>
