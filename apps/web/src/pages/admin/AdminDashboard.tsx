...
export const AdminDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        const [sellersRes, deliveryRes, usersRes, ordersRes, healthRes] = await Promise.all([
          api.get("/api/v1/applications/admin/applications").catch(() => ({ data: { data: [] } })),
          api.get("/api/v1/applications/admin/delivery/applications").catch(() => ({ data: { data: [] } })),
        ...
        const sellersWithRole = sellers.map((s: any) => ({ ...s, appType: 'Seller' }));
        const deliveryWithRole = delivery.map((d: any) => ({ ...d, appType: 'Delivery Partner' }));
        ...
        const totalSellersCount = sellers.filter((s: any) => s.status === 'approved').length;
        const totalDeliveryCount = delivery.filter((d: any) => d.status === 'approved').length;
        const pendingSellersCount = sellers.filter((s: any) => s.status === 'pending').length;
        const pendingDeliveryCount = delivery.filter((d: any) => d.status === 'pending').length;
        const totalCustomersCount = users.filter((u: any) => u.role === 'customer').length;

        // Compute GMV (Gross Merchandise Value) from all orders
        const gmvValue = orders.reduce((sum: number, o: any) => sum + parseFloat(o.total || 0), 0);
        const formattedGMV = new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR' }).format(gmvValue);

        // Pending returns / refunds / replacements count
        const pendingReturnsCount = orders.filter((o: any) => 
          ['return_requested', 'refund_requested', 'replacement_requested'].includes(o.status)
        ).length;
        ...
        setRecentActivity(activityList.length > 0 ? activityList.slice(0, 5) : []);
    ...
    <div className="flex flex-col gap-8 max-w-7xl mx-auto pb-10">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">High-level metrics and real-time platform status.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card className="bg-white border-slate-200 shadow-sm p-6">
          ...
          {loadingApps ? (
            ...
            <div>
              <h3 className="text-2xl font-extrabold text-slate-900">{metrics.gmv}</h3>
              <span className="text-xs text-slate-500 mt-1 block">Total gross merchandise value</span>
            </div>
          ) : (
        ...
        <Card className="bg-white border-slate-200 shadow-sm p-6">
          ...
          {loadingApps ? (
            ...
            <div>
              <h3 className="text-2xl font-extrabold text-slate-900">{metrics.totalOrders}</h3>
              <span className="text-xs text-slate-500 mt-1 block">Lifetime platform orders</span>
            </div>
          ) : (
            <p className="text-sm text-slate-400 italic">No data available</p>
          )}
        ...
        <Card className="bg-white border-slate-200 shadow-sm p-6">
          ...
          {loadingApps ? (
            ...
            <div>
              <h3 className="text-2xl font-extrabold text-slate-900">{metrics.totalCustomers}</h3>
              <span className="text-xs text-slate-500 mt-1 block">Total registered customers</span>
            </div>
          ) : (
            <p className="text-sm text-slate-400 italic">No data available</p>
          )}
        ...
        <Card className="bg-white border-slate-200 shadow-sm p-6">
          ...
          {loadingApps ? (
            renderCardLoader()
          ) : metrics && (metrics.totalSellers !== null || metrics.totalPartners !== null) ? (
            <div>
              <div className="flex items-baseline gap-2">
                <h3 className="text-lg font-extrabold text-slate-900">{metrics.totalSellers}</h3>
                <span className="text-xs text-slate-500">Sellers</span>
              </div>
              <div className="flex items-baseline gap-2 mt-1">
                <h3 className="text-lg font-extrabold text-slate-900">{metrics.totalPartners}</h3>
                <span className="text-xs text-slate-500">Delivery</span>
              </div>
            </div>
          ) : (
            <p className="text-sm text-slate-400 italic mt-2">No data available</p>
          )}
        </Card>
      </div>
            ...
            <CardContent className="p-0">
              {loadingApps ? (
                <div className="flex items-center justify-center p-12 text-slate-500">
                  ...
                  <span className="text-sm font-medium">Loading pending actions...</span>
                </div>
              ) : pendingActions ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 divide-y sm:divide-y-0 sm:divide-x divide-slate-100">
                  <Link to="/admin/sellers" className="p-6 hover:bg-slate-50 transition-colors group">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-bold text-slate-900 group-hover:text-indigo-600 transition-colors">Seller Applications</p>
                          <p className="text-xs text-slate-500">
                            {pendingActions.sellers !== null ? `${pendingActions.sellers} pending review` : "No data available"}
                          </p>
                        </div>
                      </div>
                  ...
                  <Link to="/admin/partners" className="p-6 hover:bg-slate-50 transition-colors group">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-bold text-slate-900 group-hover:text-indigo-600 transition-colors">Delivery Applications</p>
                          <p className="text-xs text-slate-500">
                            {pendingActions.delivery !== null ? `${pendingActions.delivery} pending review` : "No data available"}
                          </p>
                        </div>
                      </div>
                      <ArrowRight className="h-4 w-4 text-slate-400 group-hover:text-indigo-600 group-hover:translate-x-1 transition-all" />
                  ...
                  <Link to="/admin/products" className="p-6 hover:bg-slate-50 transition-colors group border-t sm:border-t-slate-100 border-slate-100">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-bold text-slate-900 group-hover:text-indigo-600 transition-colors">Product Reports</p>
                          <p className="text-xs text-slate-500">
                            {pendingActions.products !== null ? `${pendingActions.products} pending audits` : "No data available"}
                          </p>
                        </div>
                      </div>
                      <ArrowRight className="h-4 w-4 text-slate-400 group-hover:text-indigo-600 group-hover:translate-x-1 transition-all" />
                  ...
                  <Link to="/admin/orders" className="p-6 hover:bg-slate-50 transition-colors group border-t sm:border-t-slate-100 border-slate-100">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-bold text-slate-900 group-hover:text-indigo-600 transition-colors">Return Requests</p>
                          <p className="text-xs text-slate-500">
                            {pendingActions.returns !== null ? `${pendingActions.returns} pending approvals` : "No data available"}
                          </p>
                        </div>
                      </div>
                      <ArrowRight className="h-4 w-4 text-slate-400 group-hover:text-indigo-600 group-hover:translate-x-1 transition-all" />
                ...
                <div className="p-8">
                  {renderEmptyState("Pending actions metrics not available yet")}
                </div>
              )}
            </CardContent>
          </Card>
                        ...
                        >
                          <td className="px-6 py-4">
                            <div className="font-semibold text-slate-800">
                              {app.user?.name || app.shopName || app.vehicleNumber || 'Unknown'}
                            </div>
                          ...
                          <td className="px-6 py-4">
                            <span className={`px-2 py-1 rounded-md text-[10px] font-bold tracking-wider ${
                              app.appType === 'Seller' ? 'bg-purple-50 text-purple-700' : 'bg-blue-50 text-blue-700'
                            }`}>
                              {app.appType}
                            </span>
                          </td>
          ...
          <Card className="bg-white border-slate-200 shadow-sm">
            ...
            <CardContent className="space-y-4">
              {loadingApps ? (
                <div className="flex items-center justify-center p-8 text-slate-500">
                  <Loader2 className="h-5 w-5 animate-spin text-indigo-500 mr-2" />
                  <span className="text-xs font-medium">Checking systems...</span>
                </div>
              ) : healthStatus ? (
                <>
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`text-xs font-bold px-2 py-1 rounded ${
                      healthStatus.api === 'Operational' ? 'text-emerald-600 bg-emerald-50' : 'text-rose-600 bg-rose-50'
                    }`}>{healthStatus.api}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 text-sm text-slate-600">
                    ...
                    <span className={`text-xs font-bold px-2 py-1 rounded ${
                      healthStatus.database === 'Operational' ? 'text-emerald-600 bg-emerald-50' : 'text-rose-600 bg-rose-50'
                    }`}>{healthStatus.database}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 text-sm text-slate-600">
                    ...
                    <span className="text-xs font-bold text-emerald-600 bg-emerald-50 px-2 py-1 rounded">{healthStatus.jobs}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 text-sm text-slate-600">
                    ...
                    <span className="text-xs font-bold text-emerald-600 bg-emerald-50 px-2 py-1 rounded">{healthStatus.queue}</span>
                  </div>
                </>
              ) : (
