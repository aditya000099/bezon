...
export const AdminDeliveries: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'pending' | 'approved' | 'suspended' | 'rejected'>('pending');
  const { toast } = useToast();

  const fetchApplications = async () => {
    try {
      setLoading(true);
      const response = await api.get('/api/v1/applications/admin/delivery/applications');
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || 'Failed to approve delivery partner');
    }
  };
  ...
  const handleReject = async () => {
    ...
    try {
      const response = await api.post(`/api/v1/applications/admin/delivery/${selectedApp.id}/reject`, {
        reason: rejectReason,
      });
      if (response.data.success) {
      ...
      toast.error(err.response?.data?.message || 'Failed to reject delivery partner');
    }
  };
  ...
  const handleSuspend = async () => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || 'Failed to suspend delivery partner');
    }
  };
  ...
  const handleReactivate = async () => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || 'Failed to reactivate delivery partner');
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">Manage delivery partner accounts and applications</p>
        </div>
      </div>
      ...
      <div className="space-y-4 mt-4">
        <div className="flex space-x-2 border-b pb-2 overflow-x-auto">
          {['pending', 'approved', 'suspended', 'rejected'].map(tab => (
            <button
              ...
              className={`px-4 py-2 text-sm font-medium rounded-md whitespace-nowrap ${activeTab === tab ? 'bg-slate-900 text-white' : 'text-slate-600 hover:bg-slate-100'}`}
            >
              {tab.charAt(0).toUpperCase() + tab.slice(1)} ({applications.filter(a => a.status === tab).length})
            </button>
          ))}
        </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>{activeTab.charAt(0).toUpperCase() + activeTab.slice(1)} Delivery Partners</CardTitle>
            <CardDescription>Manage {activeTab} delivery partner accounts.</CardDescription>
          </CardHeader>
          <CardContent>
            {loading ? (
              <div className="py-8 text-center text-slate-500">Loading applications...</div>
            ) : filteredApps.length === 0 ? (
              <div className="py-8 text-center text-slate-500">No {activeTab} delivery partners found.</div>
            ) : (
          ...
          {selectedApp && (
            <div className="space-y-6 py-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <h5 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Emergency Contact</h5>
                  <p className="font-medium">{selectedApp.emergencyContactName}</p>
                  <p className="text-sm text-slate-600">{selectedApp.emergencyContactPhone}</p>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <h5 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Identity Documents</h5>
                  <p className="text-sm"><span className="font-medium">Aadhaar:</span> {selectedApp.aadhaarNumber}</p>
                  <p className="text-sm"><span className="font-medium">PAN:</span> {selectedApp.panNumber}</p>
                  <p className="text-sm"><span className="font-medium">License:</span> {selectedApp.drivingLicense || 'N/A'}</p>
                </div>
                <div>
                  <h5 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Vehicle Details</h5>
                  <p className="text-sm capitalize"><span className="font-medium">Type:</span> {selectedApp.vehicleType}</p>
                  <p className="text-sm"><span className="font-medium">Reg. Number:</span> {selectedApp.vehicleNumber || 'N/A'}</p>
                </div>
              </div>

              <div>
                <h5 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Address</h5>
                ...
                <div className="flex gap-2">
                  {selectedApp?.status === 'pending' && (
                    <>
                      ...
                      <Button className="bg-emerald-600 hover:bg-emerald-700 text-white" onClick={handleApprove}>Approve Partner</Button>
                    </>
                  )}
                  {selectedApp?.status === 'approved' && (
                    <Button variant="destructive" onClick={handleSuspend}>Suspend Partner</Button>
                  )}
                  {selectedApp?.status === 'suspended' && (
                    <Button className="bg-emerald-600 hover:bg-emerald-700 text-white" onClick={handleReactivate}>Reactivate Partner</Button>
                  )}
                </div>
              </>
            )}
