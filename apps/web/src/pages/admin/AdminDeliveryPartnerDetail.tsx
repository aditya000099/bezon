  ...
  const fetchDetail = async () => {
    try {
      ...
      const response = await api.get(
        API_ENDPOINTS.adminDeliveries.partnerDetail(id as string),
      );
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-bold">
                {history.returnsCompleted.length}
              </span>
            </div>
            <p className="text-xs text-zinc-400 mt-2">
              Success rate is calculated as: Completed Deliveries ÷ Total
              Assigned Deliveries
            </p>
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-3">
            {activeAssignments.length === 0 ? (
              ...
              activeAssignments.map((a: any, i: number) => (
                <div
                  key={i}
                  className="border p-3 rounded-lg flex items-center justify-between text-sm"
                >
                  <div className="flex items-center gap-2">
                    {a.type === "delivery" ? (
                      <TruckIcon className="text-blue-500" />
                    ) : (
                      <ArrowUUpLeftIcon className="text-amber-500" />
                    )}
                    <div>
                      <p className="font-semibold">
                        {a.type === "delivery" ? "Delivery" : "Return Pickup"}
                      </p>
                  ...
                  <Link
                    to={`/admin/deliveries/${a.type === "delivery" ? a.order?.id : a.id}`}
                  >
