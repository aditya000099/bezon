...
export const AdminPartners: React.FC = () => {
  ...
  const handleReactivate = async () => {
    ...
    try {
      ...
      toast.error(
        err.response?.data?.message || "Failed to reactivate partner",
      );
    }
  };

  const filteredApps = applications.filter(
    (app) => app.status === appActiveTab,
  );
    ...
    <div className="space-y-6 pb-10">
      <div>
        ...
        <p className="text-muted-foreground">
          Manage delivery partner applications and approvals.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          ...
          <Card>
            <CardHeader>
              <CardTitle>
                {appActiveTab.charAt(0).toUpperCase() + appActiveTab.slice(1)}{" "}
                Applications
              </CardTitle>
            </CardHeader>
            <CardContent>
              {filteredApps.length === 0 ? (
                <div className="py-8 text-center text-zinc-500">
                  No {appActiveTab} delivery applications found.
                </div>
              ) : (
            ...
            <div className="space-y-6 py-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  {selectedApp.vehicleType} ({selectedApp.vehicleNumber})
                </div>
                <div className="col-span-2">
                  <span className="text-zinc-500 font-bold uppercase">
                    Address:
                  </span>{" "}
                  {selectedApp.addressLine}, {selectedApp.city}
                </div>
              </div>
              {showRejectInput && (
