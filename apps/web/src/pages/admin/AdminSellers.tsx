...
export const AdminSellers: React.FC = () => {
  ...
  const handleReject = async () => {
    ...
    try {
      const response = await api.post(
        `/api/v1/applications/admin/${selectedApp.id}/reject`,
        {
          reason: rejectReason,
        },
      );
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4 mt-4">
        ...
        <Card>
          <CardHeader>
            <CardTitle>
              {activeTab.charAt(0).toUpperCase() + activeTab.slice(1)} Sellers
            </CardTitle>
            <CardDescription>
              Manage {activeTab} seller accounts.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {loading ? (
