    ...
    <Dialog open={!!selectedOrderId} onOpenChange={(open) => !open && setSelectedOrderId(null)}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Complete platform metadata records and timeline status matching for this transaction.
          </DialogDescription>
        </DialogHeader>
        ...
        {loadingDetails ? (
          ...
          <div className="space-y-6 py-2 text-zinc-700">
            ...
            <div className="border rounded-xl bg-zinc-50/50 p-2">
              <OrderTrackingStepper currentStatus={selectedOrderDetails.status} />
            </div>
              ...
              <div className="md:col-span-2 space-y-6">
                ...
                <Card className="p-4 bg-white border-zinc-200">
                  ...
                  {selectedOrderDetails.delivery?.partner ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <p className="text-xs text-zinc-400">Assigned Partner</p>
                      ...
                      <div>
                        ...
                        <p className="font-semibold text-zinc-800 mt-0.5">
                          {selectedOrderDetails.delivery.partner.user?.phone || 'No phone recorded'}
                        </p>
                      </div>
                      <div>
                        ...
                        <span className="inline-block px-2 py-0.5 text-[10px] font-bold bg-blue-50 text-blue-700 border border-blue-200 rounded mt-1 whitespace-nowrap">
                          {formatStatusText(selectedOrderDetails.delivery.status)}
                        </span>
                      </div>
                      {selectedOrderDetails.delivery.deliveredAt && (
                        <div>
                          ...
                          <p className="text-sm font-semibold text-zinc-800 mt-0.5">
                            {new Date(selectedOrderDetails.delivery.deliveredAt).toLocaleString()}
                          </p>
                        </div>
                      )}
                    </div>
                ...
                <Card className="bg-white border-zinc-200 overflow-hidden">
                  <div className="px-4 py-3 bg-zinc-50 border-b border-zinc-150">
                    <h4 className="text-xs font-bold text-zinc-500 uppercase tracking-wider">Order Items</h4>
                  </div>
                    ...
                    {selectedOrderDetails.items?.map((item: any) => (
                      <div key={item.id} className="p-4 flex items-center justify-between text-sm">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-semibold text-zinc-800">{item.productTitle}</p>
                            <p className="text-xs text-zinc-400 font-mono">SKU: {item.sku}</p>
                          </div>
                        </div>
                        <div className="text-right">
                          <p className="font-bold text-zinc-800">
                            {formatCurrency(parseFloat(item.unitPrice))} x {item.qty}
                          </p>
                  ...
                  <div className="bg-zinc-50/50 p-4 border-t text-sm space-y-1.5">
                    <div className="flex justify-between text-zinc-500 font-medium">
                      <span>Subtotal</span>
                      <span>{formatCurrency(parseFloat(selectedOrderDetails.subtotal))}</span>
                    </div>
                    {parseFloat(selectedOrderDetails.shippingCharge) > 0 && (
                      <div className="flex justify-between text-zinc-500 font-medium">
                        <span>Shipping</span>
                        <span>{formatCurrency(parseFloat(selectedOrderDetails.shippingCharge))}</span>
                      </div>
                    )}
                    {parseFloat(selectedOrderDetails.discount) > 0 && (
                      <div className="flex justify-between text-rose-500 font-medium">
                        <span>Discount ({selectedOrderDetails.couponCode || 'Promo'})</span>
                        <span>-{formatCurrency(parseFloat(selectedOrderDetails.discount))}</span>
                      </div>
                    )}
                    <div className="flex justify-between text-zinc-800 font-extrabold text-base pt-1.5 border-t">
                      <span>Grand Total</span>
                      <span>{formatCurrency(parseFloat(selectedOrderDetails.total))}</span>
                    </div>
                  </div>
                </Card>
              </div>
                  ...
                  <div className="relative border-l border-zinc-200 pl-4 space-y-5 flex-1">
                    {selectedOrderDetails.timeline?.map((evt: any) => (
                      <div key={evt.id} className="relative text-xs">
                        {/* Dot marker */}
                        <span className="absolute -left-[21px] top-1 h-2 w-2 rounded-full bg-indigo-500 ring-4 ring-white" />
                        <div className="flex items-center justify-between text-zinc-400 font-bold mb-1">
                          ...
                          <span>
                            ...
                            {new Date(evt.createdAt).toLocaleTimeString(undefined, {
                              hour: '2-digit',
                              minute: '2-digit',
                            })}
                          </span>
                        </div>
                        <p className="text-zinc-600 font-semibold mt-0.5">{evt.note || 'Status updated.'}</p>
                      </div>
                    ))}

                    {(!selectedOrderDetails.timeline || selectedOrderDetails.timeline.length === 0) && (
                      <p className="text-xs text-zinc-400 italic">No timeline entries recorded.</p>
                    )}
                  </div>
                </Card>
              </div>
