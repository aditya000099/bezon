...
export const SellerShopPage: React.FC = () => {
  ...
  if (!seller) {
    return (
      <div className="flex flex-col items-center justify-center min-h-100 text-slate-400 text-center gap-4">
        <Store className="h-16 w-16 text-slate-200" />
        <h2 className="text-xl font-bold text-slate-700">Shop profile not found</h2>
        <Link to="/shop">
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="relative overflow-hidden bg-slate-900 text-white rounded-3xl p-6 sm:p-8 md:p-10 shadow-xl border border-slate-800">
        ...
        <div className="flex flex-col md:flex-row gap-6 items-center md:items-start relative z-10">
          ...
          <div className="h-24 w-24 sm:h-28 sm:w-28 rounded-2xl bg-slate-800 border-2 border-slate-700 flex items-center justify-center text-slate-400 overflow-hidden shadow-inner shrink-0">
            {seller.logoUrl ? (
              <img
                src={seller.logoUrl}
                alt={seller.shopName}
              ...
              <Store className="h-12 w-12 text-slate-500" />
            )}
          </div>
          ...
          <div className="flex-1 text-center md:text-left flex flex-col gap-2">
            ...
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 mt-2 text-xs text-slate-400">
              ...
              <span className="flex items-center gap-1">
                ...
                {seller.products.length} published item{seller.products.length !== 1 ? "s" : ""}
              </span>
            </div>
          </div>
        </div>
      ...
      {/* Main Grid: Search & Categories + Products */}
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <Card className="bg-white border border-slate-200 shadow-sm rounded-2xl">
            ...
            <CardContent className="p-2 pt-0 flex flex-col gap-1">
              ...
              {uniqueCategories.map((cat) => {
                const count = seller.products.filter((p) => p.categoryId === cat.id).length;
                return (
                  <button
                    key={cat.id}
                  ...
                  >
                    <span>{cat.name}</span>
                    <span className="text-[10px] opacity-75 font-normal bg-slate-100 text-slate-500 rounded px-1.5 py-0.5">
                      {count}
                    </span>
                  </button>
                );
        ...
        <div className="lg:col-span-3 flex flex-col gap-10">
          {filteredProducts.length === 0 ? (
            <div className="flex flex-col items-center justify-center min-h-60 text-slate-400 border border-dashed border-slate-200 rounded-3xl p-8 bg-white/50 text-center">
              <ShoppingBag className="h-12 w-12 text-slate-300 mb-2" />
              <p className="font-bold text-slate-700">No matching items found</p>
              <p className="text-xs text-slate-400 mt-1">
                Try widening your keywords or checking another category selection.
              </p>
            </div>
          ) : (
            Object.entries(groupedProducts).map(([catId, { categoryName, items }]) => (
              <div key={catId} className="flex flex-col gap-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                  {items.map((p) => (
                    <Card
                      key={p.id}
                      className="overflow-hidden border border-slate-200 shadow-sm hover:shadow-md hover:border-slate-300 transition-all flex flex-col justify-between bg-white relative rounded-2xl"
                    >
                      <button
                        onClick={(e) => {
                          e.preventDefault();
                        ...
                        className="absolute top-3 right-3 z-10 h-7 w-7 bg-white/90 hover:bg-white text-slate-400 hover:text-rose-500 border border-slate-100 rounded-full flex items-center justify-center shadow-xs backdrop-blur-xs transition-colors"
                      >
                        ...
                        <div className="aspect-square bg-slate-50 border-b border-slate-100 flex items-center justify-center text-slate-300 font-semibold text-xs select-none cursor-pointer overflow-hidden">
                          {(() => {
                            ...
                            return primaryImg ? (
                              ...
                              <ShoppingBag className="h-10 w-10 opacity-40 text-slate-400" />
                            );
                          })()}
                        </div>
                      </Link>
                      ...
                      <CardHeader className="p-4 pb-0">
                        ...
                        <Link to={`/shop/products/${p.slug}`}>
                          <CardTitle className="text-sm font-bold text-slate-800 line-clamp-1 mt-0.5 hover:text-indigo-600 transition-colors">
                            {p.title}
                          </CardTitle>
                        </Link>
