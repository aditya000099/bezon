        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {wishlistItems.map((item) => {
            ...
            return (
              <Card
                key={item.id}
                className="overflow-hidden border border-slate-200 shadow-sm hover:shadow-md hover:border-slate-300 transition-all flex flex-col justify-between bg-white relative group"
              >
                {/* Floating Heart Button to Remove */}
                <button
                  onClick={() => toggleWishlist(p.id, p.title)}
                  className="absolute top-2.5 right-2.5 z-10 h-7 w-7 bg-white/90 hover:bg-white text-rose-500 border border-rose-100 rounded-full flex items-center justify-center shadow-sm backdrop-blur-xs transition-transform hover:scale-105 active:scale-95"
                ...
                <Link to={`/shop/products/${p.slug}`}>
                  <div className="aspect-square bg-slate-50 border-b border-slate-100 flex items-center justify-center overflow-hidden select-none cursor-pointer relative">
                    {primaryImg ? (
                      <img
                        src={primaryImg.url}
                        alt={p.title}
                      ...
                      <ShoppingBag className="h-10 w-10 text-slate-300 opacity-40" />
                    )}
                    {isOutOfStock && (
                      <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center text-white text-xs font-bold uppercase tracking-wider">
                        Out of Stock
                      </div>
                    )}
                  </div>
                ...
                <CardHeader className="p-4 pb-0">
                  ...
                  <Link to={`/shop/products/${p.slug}`}>
                    <CardTitle className="text-sm font-bold text-slate-800 line-clamp-1 mt-1 hover:text-indigo-600 transition-colors">
                      {p.title}
                    </CardTitle>
                  </Link>
                ...
                <CardFooter className="p-4 pt-0 flex gap-2 border-t border-slate-50 mt-auto">
                  <Link to={`/shop/products/${p.slug}`} className="flex-1">
                    <Button
                      variant="outline"
                      size="sm"
                      className="w-full text-xs font-bold text-slate-600 border-slate-200 hover:text-indigo-600 hover:border-indigo-200 transition-colors gap-1"
                    >
                  ...
                  {!isOutOfStock ? (
                    ...
                    <Button
                      variant="ghost"
                      className="flex-1 text-xs font-bold text-slate-400 bg-slate-50 cursor-not-allowed hover:bg-slate-50"
                      size="sm"
                      disabled
                    >
                      Unavailable
