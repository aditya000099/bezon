...
export const DeliveryQueue: React.FC = () => {
    ...
    try {
      ...
      if (queueRes.data.success) {
        ...
        const active = tasks.find((t: any) =>
          ["accepted", "picked_up", "in_transit", "out_for_delivery"].includes(t.status)
        );
  ...
  const handleAcceptTrip = async (task: any) => {
    try {
      const res = await api.patch(API_ENDPOINTS.delivery.updateStatus(task.id), {
        status: "accepted",
  ...
  const handleConfirmPickup = async () => {
    ...
    try {
      const res = await api.patch(API_ENDPOINTS.delivery.updateStatus(activeTrip.id), {
        status: "picked_up",
  ...
  const handleSimulatePhoto = () => {
    ...
    setTimeout(() => {
      ...
      setProofImageUrl("https://s3.ap-south-1.amazonaws.com/bezon-mock-sandbox/uploads/mock_proof.jpg");
      setPhotoCaptured(true);
      setIsUploading(false);
    ...
    <div className="flex flex-col gap-6 max-w-md mx-auto">
      ...
      <div className="flex justify-between items-center bg-white border border-slate-200 rounded-2xl p-4 shadow-sm">
        <div className="flex items-center gap-3">
          <div
            className={`h-3 w-3 rounded-full ${isAvailable ? "bg-emerald-500 animate-pulse" : "bg-slate-300"}`}
          ></div>
          <div>
            ...
            <p className="text-[10px] text-slate-400 mt-0.5">
              {isAvailable ? "You are visible to local order assigners" : "Go online to receive jobs"}
            </p>
          </div>
        </div>
        <Button
      ...
      {activeTrip ? (
        <div className="flex flex-col gap-3">
          <h3 className="font-extrabold text-slate-800 text-sm tracking-wider uppercase flex items-center gap-1.5 pl-1">
            <span className="h-2 w-2 bg-indigo-600 rounded-full animate-ping"></span>
            Current Active Assignment
          </h3>
          <Card className="border-indigo-100 bg-indigo-50/5 shadow-md rounded-2xl overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300">
            <CardHeader className="border-b border-indigo-50/50 pb-4 p-5">
              <div className="flex justify-between items-center">
                <CardTitle className="text-base font-extrabold text-slate-900">
                  {activeTrip.order?.orderNumber}
                </CardTitle>
                <span className="bg-indigo-50 text-indigo-700 border border-indigo-100 text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider">
                  {activeTrip.status.replace(/_/g, " ")}
                </span>
              </div>
              <CardDescription className="text-xs font-semibold text-slate-500 mt-1.5 flex items-center gap-1">
            ...
            <CardContent className="p-5 space-y-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-0.5 leading-relaxed">
                    {activeTrip.order?.seller?.addressLine}, {activeTrip.order?.seller?.city}, {activeTrip.order?.seller?.state}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3 border-t border-slate-100 pt-3.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-0.5 leading-relaxed">
                    {activeTrip.order?.addressSnapshot?.line1}, {activeTrip.order?.addressSnapshot?.city}, {activeTrip.order?.addressSnapshot?.state}
                  </p>
                  {activeTrip.order?.customer?.phone && (
                    <a
                      href={`tel:${activeTrip.order.customer.phone}`}
                      className="inline-flex items-center gap-1.5 text-indigo-600 hover:text-indigo-700 text-xs font-bold mt-2 bg-indigo-50 px-2.5 py-1 rounded-lg border border-indigo-100"
                    >
            ...
            <CardFooter className="flex gap-3 border-t border-slate-100 p-4 bg-white/50">
              {activeTrip.status === "accepted" ? (
                <Button className="w-full rounded-xl font-bold" onClick={handleConfirmPickup}>
      ...
      <div className="flex flex-col gap-3">
        <h3 className="font-extrabold text-slate-800 text-sm tracking-wider uppercase pl-1">
        ...
        {queueTasks.length === 0 ? (
          ...
          <div className="flex flex-col gap-4">
            {queueTasks.map((t) => (
              <Card
                key={t.id}
                className="bg-white border-slate-200 shadow-sm rounded-2xl overflow-hidden flex flex-col justify-between"
              >
                <CardHeader className="p-5 pb-3">
                  ...
                  <CardDescription className="text-xs font-semibold text-slate-500 mt-1 flex items-center gap-1">
                ...
                <CardContent className="p-5 pt-0 pb-4 space-y-2.5">
                  <div className="text-xs text-slate-600 leading-relaxed">
                    <strong className="text-slate-800 font-bold block mb-0.5">Pickup:</strong> 
                    {t.order?.seller?.shopName} — {t.order?.seller?.addressLine}, {t.order?.seller?.city}
                  </div>
                  <div className="text-xs text-slate-600 leading-relaxed border-t border-slate-50 pt-2">
                    <strong className="text-slate-800 font-bold block mb-0.5">Drop:</strong> 
                    {t.order?.customer?.name} — {t.order?.addressSnapshot?.line1}, {t.order?.addressSnapshot?.city}
                  </div>
                </CardContent>
      ...
      <Dialog open={showProofModal} onOpenChange={setShowProofModal}>
        <DialogContent className="max-w-md bg-white border border-slate-200 shadow-2xl p-6 rounded-2xl">
          ...
          <div className="my-6 flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-2xl p-6 bg-slate-50/50 min-h-[160px]">
            {photoCaptured ? (
              <div className="flex flex-col items-center gap-2 text-center text-emerald-600 animate-in zoom-in-95 duration-200">
                ...
                <p className="text-[9px] text-slate-400 leading-tight">
                  Timestamp: {new Date().toLocaleTimeString()} <br /> Location Coordinates Captured
                </p>
              </div>
            ) : (
              <div className="flex flex-col items-center gap-3.5 text-center">
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  <label className="inline-flex items-center justify-center rounded-xl font-bold text-xs border border-indigo-200 bg-white hover:bg-slate-50 text-indigo-600 px-4 py-2 cursor-pointer shadow-sm">
                  ...
                  <Button
                    size="sm"
                    variant="ghost"
                    className="font-bold text-[10px] text-slate-400 hover:text-slate-600"
