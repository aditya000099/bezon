...
import { Truck, MapPin, Navigation, ShieldCheck, CheckCircle2, Camera, AlertCircle } from 'lucide-react';
...
export const DeliveryQueue: React.FC = () => {
  ...
  const [queueTasks, setQueueTasks] = useState([
    { id: 'BZN-ORD-9021', seller: 'Acoustic Labs', items: '1x Premium Wireless Headphones', status: 'ready_for_pickup', pickup: 'Acoustic Warehouse, Sector 4', drop: 'Flat 102, Block C, Green Apartments, Mumbai' },
    { id: 'BZN-ORD-4491', seller: 'Sartorial Goods', items: '2x Minimalist Leather Wallet', status: 'ready_for_pickup', pickup: 'Sartorial Suite 10, Industrial Estate', drop: 'B-704, Skyline Towers, Link Road, Mumbai' }
  ]);
    ...
    <div className="flex flex-col gap-6 max-w-4xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center bg-white border border-slate-200 rounded-xl p-4 gap-4 shadow-sm">
        <div className="flex items-center gap-3">
          <div className={`h-3.5 w-3.5 rounded-full ${isAvailable ? 'bg-emerald-500 animate-pulse' : 'bg-slate-300'}`}></div>
          <div>
            <p className="text-sm font-bold text-slate-800">
              Courier Availability: {isAvailable ? 'ONLINE & ACTIVE' : 'OFFLINE'}
            </p>
            <p className="text-xs text-slate-500">You are visible to dispatch routing coordinators.</p>
          </div>
        </div>
        <Button
          ...
          <Card className="border-indigo-100 bg-indigo-50/10 shadow-md">
            <CardHeader className="border-b border-indigo-50/55 pb-4">
              <div className="flex justify-between items-center">
                <CardTitle className="text-base font-extrabold text-slate-900">{activeTrip.id}</CardTitle>
              ...
              <CardDescription className="text-xs font-medium text-slate-500 mt-1">
                Fulfilling for: <strong className="text-slate-700">{activeTrip.seller}</strong> ({activeTrip.items})
              </CardDescription>
            </CardHeader>
            <CardContent className="pt-6 space-y-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Pickup Warehouse</p>
                  <p className="text-sm font-semibold text-slate-700 mt-0.5">{activeTrip.pickup}</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Dropoff Destination</p>
                  <p className="text-sm font-semibold text-slate-700 mt-0.5">{activeTrip.drop}</p>
                </div>
              </div>
            </CardContent>
            <CardFooter className="flex gap-3 border-t border-slate-100 pt-4 bg-white/50 rounded-b-lg">
              {activeTrip.status === 'accepted' ? (
                ...
                <Button className="w-full font-bold bg-emerald-600 hover:bg-emerald-700 text-white" onClick={handleOpenProofOfDelivery}>
      ...
      <div className="flex flex-col gap-4">
        <h3 className="font-extrabold text-slate-800 text-lg">Available Dispatch Queue</h3>
        {queueTasks.length === 0 ? (
          <p className="text-slate-400 text-sm italic">No pending packages await allocation.</p>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {queueTasks.map((t) => (
              <Card key={t.id} className="bg-white border-slate-200 shadow-sm flex flex-col justify-between">
                <CardHeader>
      ...
      <Dialog open={showProofModal} onOpenChange={setShowProofModal}>
        <DialogContent className="max-w-md bg-white border border-slate-200 shadow-2xl p-6 rounded-2xl">
          <DialogHeader className="border-b border-slate-100 pb-4">
            <DialogTitle className="text-lg font-bold text-slate-900">Proof of Delivery (POD)</DialogTitle>
            <DialogDescription>
              Capture photographic proof of receipt at the customer address.
            </DialogDescription>
          </DialogHeader>

          <div className="my-6 flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-2xl p-8 bg-slate-50/50 min-h-[200px]">
            {photoCaptured ? (
              <div className="flex flex-col items-center gap-2 text-center text-emerald-600">
                ...
                <p className="text-[10px] text-slate-400">Timestamp: {new Date().toLocaleTimeString()} · Location Lock verified</p>
              </div>
            ) : (
              <div className="flex flex-col items-center gap-3 text-center">
                <Camera className="h-10 w-10 text-slate-300" />
                <p className="text-xs text-slate-500">Camera permission authorized</p>
                <Button size="sm" variant="outline" className="font-bold text-xs" onClick={handleSimulatePhoto} disabled={isUploading}>
          ...
          <DialogFooter className="flex flex-row gap-3 pt-4 border-t border-slate-100 w-full">
            <Button variant="outline" className="flex-1 font-bold text-xs" onClick={() => setShowProofModal(false)}>
              Cancel
            </Button>
            <Button
