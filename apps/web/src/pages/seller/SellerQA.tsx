...
export const SellerQA: React.FC = () => {
  ...
  const fetchQuestions = async (pageNum: number, append = false) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        if (append) {
          setQuestions((prev) => [...(Array.isArray(prev) ? prev : []), ...newQuestions]);
        } else {
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* Header */}
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
        <div>
      ...
      {/* Filters Row */}
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
        ...
        <select
          ...
          className="w-full sm:w-64 bg-white border border-slate-200 rounded-lg text-sm px-3 py-2 outline-none font-semibold text-slate-600 cursor-pointer h-10"
        >
              ...
              <CardHeader className="pb-3">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <CardTitle className="text-sm font-bold text-slate-800 mt-0.5 leading-snug">
                      {q.question}
                    </CardTitle>
              ...
              <CardContent className="space-y-3">
                ...
                <div className="flex gap-2 pt-2 border-t border-slate-100">
                  <textarea
                    ...
                    rows={2}
                    className="flex-1 bg-white border border-slate-200 rounded-lg text-sm px-3 py-2 outline-none text-slate-700 placeholder:text-slate-400 resize-none focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100 transition-all"
                  />
                  <Button
                    size="sm"
